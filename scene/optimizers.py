...
class SparseGaussianAdam(BaseAdam):
    ...
    def add_and_prune(
        ...
        if submap_id is not None:
            new_submap_ids = torch.full(
                (extension_tensors["xyz"].shape[0], ), 
                submap_id, 
                dtype=torch.int32, device="cuda"
            )
        else:
            new_submap_ids = torch.empty(0, dtype=torch.int32, device="cuda")
            
        self.submap_ids = torch.cat(
            [
                self.submap_ids[valid_mask],
                new_submap_ids
            ], 
            dim=0
        )
        
        # Return the updated submap_ids so SceneModel can update its reference
        return self.submap_ids
