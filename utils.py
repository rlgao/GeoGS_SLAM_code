...
def get_vggt_image_size(image_paths):
...
def get_lapla_norm(img, kernel):
    ...
    laplacian_kernel = laplacian_kernel.repeat(
        1, img.shape[0], 1, 1
    )  # [1, C, 3, 3]
...
def create_gaussian_kernel(sigma, size=None, device=None, dtype=None):
    if size is None:
        size = int(6 * sigma + 1)  # 6*sigma covers 99.7% of Gaussian
        if size % 2 == 0: 
...
def get_dog_norm(img, kernel, sigma1=0.5, sigma2=1.5):
    """
    Compute the norm of Difference of Gaussians (DoG) filter applied to the input image.
    
    Args:
        img: Input image tensor [C, H, W]
        kernel: Smoothing kernel for final convolution [1, 1, K, K]
        sigma1: Standard deviation of the first Gaussian (smaller)
        sigma2: Standard deviation of the second Gaussian (larger)
    ...
    g1 = create_gaussian_kernel(
        sigma1, 
        device=device, dtype=dtype
    ).repeat(1, img.shape[0], 1, 1)  # [1, C, size, size]
    g2 = create_gaussian_kernel(
        sigma2, 
        device=device, dtype=dtype
    ).repeat(1, img.shape[0], 1, 1)  # [1, C, size, size]

    blur1 = F.conv2d(img[None], g1, padding="same")
    blur2 = F.conv2d(img[None], g2, padding="same")
    dog_response = blur1 - blur2  # [1, 1, H, W]

    dog_norm = torch.linalg.vector_norm(
        dog_response, ord=1, dim=1, keepdim=True
    )
    for i in range(4):
        dog_norm[..., :, i] = 0
        dog_norm[..., :, -1-i] = 0
    ...
    # return dog_norm[0, 0].clamp(0, 1)  # [H, W]
    return F.conv2d(
        dog_norm, 
        kernel, 
        padding="same"
...
def unitquat2mtx(quat):
    w, x, y, z = quat
    rotation_so3 = torch.stack([
...
def mtx2unitquat(R):
    # Convert SO(3) rotation matrix back to unit quaternion (w, x, y, z)
    ...
    if trace > 0.0:
        ...
        if R[0, 0] > R[1, 1] and R[0, 0] > R[2, 2]:
            ...
            y_new = (R[1, 2] + R[2, 1]) / S
            z_new = 0.25 * S
    quat_new = torch.stack([w_new, x_new, y_new, z_new])
    quat_new = quat_new / torch.linalg.norm(quat_new)
    return quat_new
