...
class PoseGraph:
    ...
    def add_prior_factor(self, key, pose, noise):
        ...
        if key not in self.initialized_nodes:
            raise ValueError(
                f"Trying to add prior factor for key {key} but it is not in the graph."
            )
        
        self.graph.add(
            PriorFactorPose3(key, Pose3(pose), noise)
        )
    ...
    def add_between_factor(self, key1, key2, relative_pose, noise):
        ...
        if key1 not in self.initialized_nodes or key2 not in self.initialized_nodes:
            raise ValueError(
                f"Both poses {key1} and {key2} must exist before adding a factor."
            )
