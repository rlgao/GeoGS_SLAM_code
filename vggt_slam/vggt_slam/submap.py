...
class Submap:
    def __init__(self, submap_id):
        ...
        self.poses = None  # (S, 4, 4)
        
        self.frames = None  # torch.Size([S, 3, H, W])
        self.intrinsics = None  # (S, 3, 3)
        self.retrieval_vectors = None
        self.colors = None  # (S, H, W, 3)
        self.depths = None  # (S, H, W, 1)
        self.conf = None  # (S, H, W)
        self.conf_masks = None  # (S, H, W)
        self.conf_threshold = None
        
        self.pointclouds = None  # (S, H, W, 3), from each camera
        self.voxelized_points = None
        
        self.last_non_loop_frame_index = None
        self.frame_ids = None  # does not include any of the loop closure frames
    ...
    def get_all_frames(self, ignore_loop_closure_frames=True):
        if self.last_non_loop_frame_index is None:
            return self.frames
        
        non_loop_frames_num = self.last_non_loop_frame_index + 1
        if ignore_loop_closure_frames:
            return self.frames[:non_loop_frames_num]
        
        return self.frames
    
    
    def get_all_pointmaps(self, ignore_loop_closure_frames=True):
        if self.last_non_loop_frame_index is None:
            return self.pointclouds
        
        non_loop_frames_num = self.last_non_loop_frame_index + 1
        if ignore_loop_closure_frames:
            return self.pointclouds[:non_loop_frames_num]
        
        return self.pointclouds
    
    
    def get_all_depths_confs(self, ignore_loop_closure_frames=True):
        if self.last_non_loop_frame_index is None:
            return self.depths, self.conf
        
        non_loop_frames_num = self.last_non_loop_frame_index + 1
        if ignore_loop_closure_frames:
            return self.depths[:non_loop_frames_num], self.conf[:non_loop_frames_num]
        
        return self.depths, self.conf
    
    def get_all_intrinsics(self, ignore_loop_closure_frames=True):
        if self.last_non_loop_frame_index is None:
            return self.intrinsics
        
        non_loop_frames_num = self.last_non_loop_frame_index + 1
        if ignore_loop_closure_frames:
            return self.intrinsics[:non_loop_frames_num]
    ...
    def get_all_poses_world(self, ignore_loop_closure_frames=True):
        projection_mat_list = (
            self.intrinsics 
    ...
    def get_all_poses_world2(self, ignore_loop_closure_frames=True):
        poses_world_cam = []
        
        for index, pose in enumerate(self.poses):
            pose_world_cam = self.H_world_map @ pose
            poses_world_cam.append(pose_world_cam)
            
            if ignore_loop_closure_frames and index == self.last_non_loop_frame_index:
                break
        
        return np.stack(poses_world_cam, axis=0)
    
    
    def get_frame_pointcloud(self, index):
    ...
    def get_pose_subframe(self, index):
        # cameras to first frame, (4, 4)
        return np.linalg.inv(self.poses[index])
    ...
    def get_points_list_in_world_frame(self, ignore_loop_closure_frames=True):
        point_list = []
        frame_id_list = []
        frame_conf_mask = []
        ...
        for index, points in enumerate(self.pointclouds):
            ...
            points_homogeneous = np.hstack([
                points_flat, 
                np.ones((points_flat.shape[0], 1))
            ])
            points_transformed = (self.H_world_map @ points_homogeneous.T).T
            point_list.append(
                (points_transformed[:, :3] / points_transformed[:, 3:]).reshape(points.shape)
            )
