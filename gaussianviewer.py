...
class GaussianViewer(Viewer):
    atttrs_to_sync = [
        ...
        "show_top_view", "show_gt_view", "keyframe_id", "altitude_control", "altitude_smoothing",
    ...
    def create_widgets(self):
        ...
        self.point_view_camera = FPSCamera(
            self.mode, 
            # ==========================
            width, height, 
        ...
        self.top_view_camera = FPSCamera(
            self.mode, 
            # ==========================
            # 480, 480,
            1280, 720,  # 720p
        ...
        self.cameras = {
            "top_view": self.top_view_camera, 
            "point_view": self.point_view_camera
        }
        
        self.point_view = TorchImage(self.mode)
        self.top_view = TorchImage(self.mode)
        self.gt_view = TorchImage(self.mode)
        self.views = {
            "top_view": self.top_view, 
            "point_view": self.point_view,
        ...
        # Render settings
        views = ["top_view", "point_view", "gt_view"]
        ...
        self.pose_sizes = {view: 0.04 for view in views}  # 0.1
        self.draw_anchors = {view: False for view in views}
        self.anchor_sizes = {view: 0.1 for view in views}
        self.scaling_factor = {"top_view": 0.004, "point_view": 1, "gt_view": 1}  # 0.002
        self.reset_intrinsics_flag = {view: False for view in views}
        self.bg_color = [0.0, 0.0, 0.0]
        self.show_top_view = True # False
        self.show_gt_view = True
        self.max_fps = 20
        ...
        self.snap_mode = RadioPicker(ViewerMode.LOCAL, SnapMode.last)
    ...
    def step(self):
        ...
        # Render scene
        for view in ["point_view", "top_view", "gt_view"]:
            if view == "gt_view":
                # GT view doesn't need a camera, handle it separately
                if self.show_gt_view:
                    # Get GT image from keyframe
                    if self.snap_mode.value in [SnapMode.keyframe, SnapMode.last] and len(self.scene_model.keyframes) > 0:
                        keyframe_id = self.keyframe_id if self.snap_mode.value == SnapMode.keyframe else -1
                        keyframe = self.scene_model.keyframes[keyframe_id]
                        # Get GT image from keyframe
                        gt_image = keyframe.image_pyr[0]  # [3, H, W]
                        # Resize to match a reasonable resolution (use point_view camera resolution)
                        height, width = self.point_view_camera.res_y, self.point_view_camera.res_x
                        gt_image_resized = torch.nn.functional.interpolate(
                            gt_image.unsqueeze(0), 
                            size=(height, width), 
                            mode='bilinear', 
                            align_corners=False
                        ).squeeze(0)
                        image = gt_image_resized.clamp(0, 1.0).mul(255).permute(1, 2, 0).byte()
                    else:
                        # No keyframes available, show black image
                        height, width = self.point_view_camera.res_y, self.point_view_camera.res_x
            ...
            # Draw Gaussians
            if (
                view == "point_view" and self.render_mode() in ["Splats", "Depth"]
            ) or (
                view == "top_view" and self.show_top_view
            ):
                width = camera.res_x
                height = camera.res_y
                ...
                if self.render_mode() == "Splats":
                    ...
                    # image = (1.0 / render_pkg["invdepth"][0].clamp_min(1e-8)).clamp(0, 255).byte().cpu().numpy()  # [H, W]
                    
                    image = cv2.cvtColor(
                        cv2.applyColorMap(image, cv2.COLORMAP_JET),  # COLORMAP_INFERNO
                        cv2.COLOR_BGR2RGB
                    )
                ...
                # ===============================================================
                ############## snapshot ##############
                # if self.snapshot and view == "point_view":
                if self.snapshot and view == "top_view":
                    # Save snapshot image in 'ppt/snapshot' as PNG
                    import os
                    snapshot_dir = "ppt/snapshot"
                    os.makedirs(snapshot_dir, exist_ok=True)
                    from datetime import datetime
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"snapshot_{view}_{timestamp}.png"
                    filepath = os.path.join(snapshot_dir, filename)
                    
                    # Convert image to CPU numpy array if needed
                    img_np = image.detach().cpu().numpy()
                    # If image is torch tensor and has 3 channels, ensure it's uint8
            ...
            # Draw ellipsoids
                ...
                # ===============================================================
                if self.snapshot:
                    # Save snapshot image in 'ppt/snapshot' as PNG for ellipsoid_viewer
                    import os
                    # import numpy as np
                    snapshot_dir = "ppt/snapshot"
                    os.makedirs(snapshot_dir, exist_ok=True)
                    from datetime import datetime
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"snapshot_ellipsoid_{timestamp}.png"
                    filepath = os.path.join(snapshot_dir, filename)

                    # Get rendered image from ellipsoid_viewer
                    arr, meta = self.ellipsoid_viewer.server_send()
                    
                    if arr is not None and meta is not None:
                        # Ensure arr is a numpy array
                        if isinstance(arr, bytes):
                            img_np = np.frombuffer(arr, dtype=np.uint8)
                            img_np = img_np.reshape(meta["shape"])
                        else:
                            img_np = arr
                        # If image is not uint8, convert
                        if not hasattr(img_np, "dtype") or img_np.dtype != np.uint8:
                            img_np = np.array(img_np).clip(0, 255).astype('uint8')
                        # If image is CHW, convert to HWC
                        if img_np.ndim == 3 and img_np.shape[0] in [1,3]:
                            img_np = img_np.transpose(1, 2, 0)
                        # Save using cv2
                        cv2.imwrite(filepath, cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR))
                        print(colored(
                            f"Ellipsoid snapshot saved to {filepath}",
                            'red', 'on_light_grey', ['bold']
                        ))
                    else:
                        print(colored(
                            "Ellipsoid snapshot failed: no image data.",
                            'red', 'on_light_grey', ['bold']
                        ))
                    self.snapshot = False
                # ===============================================================
    ...
    def show_gui(self):
        ...
        with imgui_ctx.begin(f"Point View Settings"):
            ...
            if self.snap_mode.value == SnapMode.keyframe:
                ...
                imgui.text(
                    f"Keyframe ID ({self.scene_model.keyframes[self.keyframe_id].frame_id})"
                )
            ...
            if self.snap_mode.value == SnapMode.last:
                if len(self.scene_model.keyframes) > 0:
                    imgui.text(
                        f"Keyframe ({len(self.scene_model.keyframes) - 1}) ID ({self.scene_model.keyframes[-1].frame_id})"
                    )
                else:
                    imgui.text(
                        f"No keyframe yet"
            ...
            self.snapshot = imgui.button("Snapshot")


        # with imgui_ctx.begin("Point View"):
        with imgui_ctx.begin("Rendered image"):
            if self.render_mode() in ["Splats", "Depth"]:
        ...
        if self.show_gt_view:
            with imgui_ctx.begin("GT keyframe image"):
                self.gt_view.show_gui()
        ...
        # GT View Settings
        with imgui_ctx.begin("GT View Settings"):
            _, self.show_gt_view = imgui.checkbox("Show GT View", self.show_gt_view)
            if self.show_gt_view:
                if self.snap_mode.value in [SnapMode.keyframe, SnapMode.last] and len(self.scene_model.keyframes) > 0:
                    keyframe_id = self.keyframe_id if self.snap_mode.value == SnapMode.keyframe else -1
                    keyframe = self.scene_model.keyframes[keyframe_id]
                    imgui.text(f"GT Image from Keyframe {keyframe_id} (Frame {keyframe.frame_id})")
                else:
                    imgui.text("No keyframes available for GT view")
    ...
    def client_send(self):
        attrs = {
            key: getattr(self, key) for key in GaussianViewer.atttrs_to_sync
        }
        return None, attrs
...
# python gaussianviewer.py local results/MipNeRF360/garden_half/
